...
public class ParseRank {
	...
	public static NewRankDay parseFromRow(Element row) {
		...
		rankDay.setDianzhan(App.parseInt(dianzhan.text()));

		String parts[] = fabu.text().split("/");
		rankDay.setFabu1(Integer.parseInt(parts[0]));
		rankDay.setFabu2(Integer.parseInt(parts[1]));

		rankDay.setPingjun(App.parseInt(pingjun.text()));
		// rankDay.setRank(i);
		rankDay.setToutiao(App.parseInt(toutiao.text()));
		rankDay.setZishu(Double.parseDouble(zishu.text()));
		rankDay.setZongyuedu(App.parseInt(zongyuedu.text()));
		rankDay.setZuigao(App.parseInt(zuigao.text()));
		return rankDay;
	}
}
